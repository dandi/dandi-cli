...
def test_IteratorWithAggregation():
    ...
    it = IteratorWithAggregation(sleeping_range(3, 0.001), agg=sumup)
    ...
    it = IteratorWithAggregation(sleeping_range(5, 0.001, thr=2), agg=sumup)
    got = []
    ...
    it = IteratorWithAggregation(
        sleeping_range(5, 0.001, thr=2), agg=sumup, reraise_immediately=True
    )
    got = []
    with pytest.raises(ValueError):
        for i in it:
